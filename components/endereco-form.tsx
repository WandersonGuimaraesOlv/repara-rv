...
export function EnderecoForm({ onAddressChange, initialNeighborhood = 'Setor Central' }: EnderecoFormProps) {
  ...
  return (
    <div className="space-y-3 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm text-left">
      <div className="flex items-center justify-between">
        <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-1.5">
          <MapPin size={14} className="text-orange-500" />
          Endereço do Atendimento (Rio Verde)
        </h3>
        <span className="text-[10px] text-gray-400">Campos obrigatórios *</span>
      </div>

      {/* Campo CEP (Opcional / Agilizador) */}
      <div className="relative">
        <label className="text-xs font-medium text-gray-600 block mb-1">
          CEP <span className="text-gray-400 font-normal">(opcional — preenche rua e bairro sozinho)</span>
        </label>
        <div className="relative">
          <input
            type="tel"
            ...
            maxLength={9}
            className="w-full px-3 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500 transition"
          />
          {loadingCep && (
            <span className="absolute right-3 top-2.5 text-xs text-orange-600 animate-pulse font-medium">
              Buscando...
            </span>
          )}
          {cepSuccess && !loadingCep && (
            <span className="absolute right-3 top-2.5 text-xs text-emerald-600 flex items-center gap-1 font-medium">
      ...
      <div className="grid grid-cols-4 gap-2">
        ...
        <div className="col-span-1">
          ...
          <input
            ...
            required
            className="w-full px-3 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500 transition text-center font-bold"
          />
        </div>
      </div>
