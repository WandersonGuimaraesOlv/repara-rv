    ...
    <div className="space-y-6">
        ...
        {LEGAL_DOCUMENT_SLUGS.map((slug) => (
          <button
            ...
            className={`px-4 py-2.5 text-sm font-semibold rounded-t-lg transition-colors ${
              activeSlug === slug
                ? 'bg-slate-900 text-orange-400 border border-slate-800 border-b-slate-900'
        ...
        <>
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 shadow-sm space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-bold text-slate-200 uppercase tracking-wide">Metadados do Documento</h2>
              {document?.updated_at && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-700 rounded-lg text-sm text-white px-3 py-2 focus:outline-none focus:border-orange-500"
                />
              </div>
              <div>
              ...
              <div>
                <label className="text-xs text-slate-400 block mb-1">Data de Vigência</label>
                <input
                  type="date"
            ...
            <button
              ...
              disabled={savingMeta}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 transition-colors disabled:opacity-50"
            >
              <Save size={14} />
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-semibold bg-orange-500 hover:bg-orange-600 text-white transition-colors"
              >
                <Plus size={14} />
                Nova Cláusula
              </button>
            ...
            {clauses.length === 0 && (
              <div className="text-sm text-slate-500 py-6 text-center border border-dashed border-slate-800 rounded-2xl">
            ...
            {clauses.map((clause, index) => {
              ...
              return (
                <div
                  key={clause.id}
                  className="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 flex items-start gap-3"
                >
                  <span className="shrink-0 w-7 h-7 rounded-full bg-slate-800 text-slate-300 text-xs font-bold flex items-center justify-center mt-0.5">
                    {index + 1}
                  </span>
                  {IconComponent && <IconComponent size={16} className="text-orange-500 shrink-0 mt-1.5" />}
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-white truncate">{clause.title}</p>
                    <p className="text-xs text-slate-500 font-mono">#{clause.section_id}</p>
                  </div>
      ...
      {editingClauseOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-7 max-w-2xl w-full shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-white">
                {clauseForm.id ? 'Editar Cláusula' : 'Nova Cláusula'}
              </h3>
              <button type="button" onClick={() => setEditingClauseOpen(false)} className="text-slate-400 hover:text-white">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-700 rounded-lg text-sm text-white px-3 py-2 font-mono focus:outline-none focus:border-orange-500"
                  placeholder="garantia-7-dias"
                />
              </div>
              <div>
                <label className="text-xs text-slate-400 block mb-1">Ícone (opcional)</label>
                <select
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs text-slate-400">Corpo (Markdown)</label>
                <button
                  type="button"
              ...
              {previewOpen ? (
                <div
                  className="legal-card bg-slate-950 border border-slate-700 rounded-lg p-4 text-sm text-slate-200 min-h-[180px] prose prose-invert prose-sm max-w-none"
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold bg-orange-500 hover:bg-orange-600 text-white transition-colors disabled:opacity-50"
              >
                <Save size={14} />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-7 max-w-md w-full shadow-2xl space-y-5">
            ...
            <p className="text-sm text-slate-400">
              Tem certeza que deseja excluir <strong className="text-slate-200">&quot;{deletingClause.title}&quot;</strong>?
